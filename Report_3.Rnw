...
This is the same as our backwards selection model!
...
Additionally, based on the VIF values for both models, we do not have a multicolinearity problem. 
<<>>=
require(car)
vif(fwdselec)
vif(bkwdselec)
...
Let us compare our backwards and forwards selection models. 
...
summary(bkwdselec)
#delete famrel and Dalc (least significant)
bkwdselec_reduced <- lm(G3 ~ school + sex + Mjob + failures + higher + health, data = col.trn)
@

<<>>=
#compare each with anova
anova(fwdselec, fwdselec_reduced)
anova(bkwdselec, bkwdselec_reduced)
@
