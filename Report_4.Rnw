...
We will now run ridge regression and LASSO on the full varaible set, using cross validation to find lambda, and compare them using SSE. 

Below is a plot comparing responses predicted using MLR, RR, and LASSO. 

Now to choose one variable and run smoothing pline and kernel smoother models on it. We change the parameters so that we have four different models for each method.
